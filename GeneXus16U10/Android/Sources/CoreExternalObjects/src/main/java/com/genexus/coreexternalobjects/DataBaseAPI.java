...
import com.artech.common.ZipHelper;
...
public class DataBaseAPI extends ExternalApi {
    ...
    private static boolean backupDB(@NonNull String outDirPath) {
		...
		try {
			...
			if (FileHelper.copyFile(inFileName, outFileName, false)) {
				...
				if (!FileHelper.writeStringAsFile(MyApplication.getApp().getReorMD5Hash(), md5File))
					return false;
	...
	private static boolean restoreMD5(@NonNull String inDirPath) {
		...
		if (md5!=null
			&& md5.equalsIgnoreCase(MyApplication.getApp().getReorMD5Hash())
		)
			return true;
