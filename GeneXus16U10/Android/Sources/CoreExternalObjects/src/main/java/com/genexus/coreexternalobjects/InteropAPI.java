...
public class InteropAPI extends ExternalApi {
	...
	public ExternalApiResult execute(String method, List<Object> parameters) {
		...
		if (method.equalsIgnoreCase("sendmessage")) {
			...
			if (openInBrowser(parameterValues.get(0)))
				return ExternalApiResult.SUCCESS_WAIT;
			else
				return getInteropActionFailureResult();
			...
			IRemoteNotificationProvider provider = RemoteNotification.getDefaultProvider();
			if (provider == null) {
				Services.Log.error(METHOD_SET_BADGE_NUMBER + " failed because there isn't a notification provider");
				return ExternalApiResult.FAILURE;
			}

			IBadgeNotification badgeNotification = Cast.as(IBadgeNotification.class, provider);
			if (badgeNotification == null) {
				Services.Log.error(METHOD_SET_BADGE_NUMBER + " failed because the notification provider doesn't support badges");
				return ExternalApiResult.FAILURE;
			}

			badgeNotification.setBadgeCount(getContext(), number);
			return ExternalApiResult.SUCCESS_CONTINUE;
		}
	...
	private boolean openInBrowser(String url) {
		Intent intent = ActivityLauncher.newCustomTabsIntent(getActivity(), Uri.parse(url));
		return PhoneHelper.startAction(getActivity(), intent);
