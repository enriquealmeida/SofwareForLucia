...
public class AudioApi extends ExternalApi
{
	...
	static void onQueueStateChanged(GxMediaQueueState state)
	{
		// Log.d(EVENT_QUEUE_STATE_CHANGED, state.toString());
		fireEvent(EVENT_QUEUE_STATE_CHANGED, state.toSdt());
	}

	static void onMediaItemFinished(GxMediaItemFinishedArgs finishedArgs)
	{
		// Log.d(EVENT_QUEUE_ITEM_FINISHED, finishedArgs.toString());
		fireEvent(EVENT_QUEUE_ITEM_FINISHED, finishedArgs.toSdt());
	}

	static void onCustomActionEvent(String actionId, GxMediaItem mediaItem)
	{
		fireEvent(EVENT_CUSTOM_ACTION, actionId, mediaItem.toSdt());
	}
	...
	private AudioPlayerBackground getBackgroundAudio()
	{
		if (getActivity() != null)
			...
			return AudioPlayerBackground.getInstance(MyApplication.getAppContext());
	}
