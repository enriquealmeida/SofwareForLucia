...
public class AudioPlayerBackground implements IAudioPlayer
{
	...
	private final MediaControllerCompat.Callback mMediaCallback = new MediaControllerCompat.Callback()
	{
		...
		public void onPlaybackStateChanged(PlaybackStateCompat state)
		{
			...
			AudioApi.onQueueStateChanged(new GxMediaQueueState(state));
		}
	};
	...
	private final BroadcastReceiver mMediaItemFinishedReceiver = new BroadcastReceiver()
	{
		...
		{
			AudioApi.onMediaItemFinished(new GxMediaItemFinishedArgs(intent));
		}
	};
	...
	private final BroadcastReceiver mCustomActionEventReceiver = new BroadcastReceiver()
	{
		...
		public void onReceive(Context context, Intent intent)
		{
			...
			if (mediaItem != null)
				AudioApi.onCustomActionEvent(actionId, mediaItem);
		}
	};
	...
	@Nullable
	public AudioPlayerThemeClassDefinition getPlayerThemeClass()
	{
		// Cannot be chosen right now, but might be.
		return Services.Themes.getThemeClass(AudioPlayerThemeClassDefinition.class, AudioPlayerThemeClassDefinition.CLASS_NAME);
	}
}
