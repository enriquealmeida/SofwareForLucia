...
class RelativeTimerFormatter(config: FormatterConfig = FormatterConfig()) {
    ...
    private fun isAboveMaxSeconds(durationSeconds: Duration<IsoUnit>): Boolean {
        return config.maxSeconds > 0 && durationSeconds.getPartialAmount(SECONDS) >= config.maxSeconds
    }

    private fun isBellowMinSeconds(durationSeconds: Duration<IsoUnit>): Boolean {
        return config.minSeconds > 0 && durationSeconds.getPartialAmount(SECONDS) <= config.minSeconds
    }

    private fun replaceClockPattern(duration: Duration<IsoUnit>, clockPattern: String, pattern: String, truncate: IsoUnit, ammount: IsoUnit): String {
