...
class CryptoHelper
{
		...
		try
		{
			...
			else
			{
				if (MyApplication.getApp() == null ||
						!MyApplication.getApp().getUseLegacyClientStorage())
				{
	...
	private static class SecretKeyWrapperLegacy implements SecretKeyWrapper
	{
		...
		@NonNull
		@Override
		public byte[] wrap(@NonNull SecretKey key) {
			return scrambleBytes(key.getEncoded());
		}

		@NonNull
		@Override
		public SecretKey unwrap(@NonNull byte[] blob) {
			return new SecretKeySpec(scrambleBytes(blob), 0, blob.length, SECRET_KEY_ALGORITHM);
		}

		@NonNull
		private static byte[] scrambleBytes(@NonNull byte[] blob)
		{
	...
	private static class SecretKeyWrapperApi18 implements SecretKeyWrapper
	{
		...
		@NonNull
		@Override
		public byte[] wrap(@NonNull SecretKey key) throws GeneralSecurityException
		{
		...
		@NonNull
		@Override
		public SecretKey unwrap(@NonNull byte[] blob) throws GeneralSecurityException
		{
