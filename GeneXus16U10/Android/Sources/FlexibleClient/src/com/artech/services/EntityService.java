...
public abstract class EntityService extends Service
{
	...
	private void startOrEnqueueTask(LoadDataTask task)
	{
		if (!task.getStatus().equals(AsyncTask.Status.PENDING))
			throw new IllegalStateException("Illegal status for task being started: " + task.getStatus());
	...
	void onTaskFinished(LoadDataTask task, String intentFilter, EntityServiceResponse response)
	{
		...
		synchronized (sSyncObject)
		{
			...
			else
			{
				// Sanity checks
				if (taskList != null && taskList.contains(task))
					throw new IllegalStateException("Task reports being cancelled, but it was in the list of running tasks! " + task);
			}
		}
	}
