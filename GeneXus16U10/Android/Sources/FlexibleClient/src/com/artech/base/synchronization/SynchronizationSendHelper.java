...
public class SynchronizationSendHelper
{
	...
	public static int callOfflineReplicator()
	{
		...
		IProcedure procedure = MyApplication.getApplicationServer(Connectivity.Online).getProcedure("GeneXus.SD.Synchronization.OfflineEventReplicator");
	...
	public static int processPendingEvents(IProcedure procedure, EntityList pendings )
	{
		...
		if (pendings.size()>0)
		{
			...
			myEntityParm.setProperty("Synchronizer", MyApplication.getApp().getSynchronizer());
			...
			if (procResult.isOk())
			{
				...
				StructureDataType defStructureOutput = MyApplication.getApp().getDefinition().getSDT("GeneXus.SD.Synchronization.SynchronizationEventResultList");
			...
			try {
				...
				if (eventFilesArray.length()>0)
				{
					...
					for(int arrayIndex = 0; arrayIndex < eventFilesArray.length(); arrayIndex++)
					{
						...
						ResultDetail<String> uploadResult = serverApp.uploadBinary(file, null);
