...
public class TabControlDefinition extends LayoutItemDefinition
{
	...
	// result in px
	private static int getTabStripHeightFromTheme(ThemeClassDefinition themeClass)
	{
		if (themeClass != null)
		{
			TabControlThemeClassDefinition tabThemeClass = Cast.as(TabControlThemeClassDefinition.class, themeClass);
			if (tabThemeClass != null)
			{
				//Services.Log.debug("TabControlDefinition getTabStripHeight theme height" + tabThemeClass.getTabStripHeight());
				if (tabThemeClass.getTabStripHeight() > 0)
					return tabThemeClass.getTabStripHeight();
			}
			else
			{
				Services.Log.warning("TabControlDefinition getTabStripHeight theme class invalid");
			}
		}
		else
		{
