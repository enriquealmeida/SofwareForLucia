...
public class ImageResizingHelper {
	 ...
	 */
	@NonNull
	public static File resizeImageForUpload(@NonNull File srcFile, int maxUploadSizeMode) throws IOException {
		...
		UploadSizeDefinition uploadSizeDef = Services.Application.getPatternSettings().getUploadSize(maxUploadSizeMode);
	...
	private static File scaleDownToDesiredDimensions(@NonNull File srcFile, @NonNull File destPath, @NonNull String destBaseName,
		...
		if (shouldResizeImage) {
			...
			*/
			...
			boolean compressionSuccessful = scaledBitmap.compress(p.first, 100, new FileOutputStream(destFile));
	...
	private static Pair<Bitmap.CompressFormat, String> getCompressFormatAndExtension(String mimeType)
	{
		...
		switch (mimeType) {
			...
			case "image/webp":
				return new Pair<>(Bitmap.CompressFormat.WEBP, "webp");
			case "image/jpeg":
	...
	@NonNull
	private static BitmapFactory.Options getImageDimensions(@NonNull File srcFile) throws IOException {
