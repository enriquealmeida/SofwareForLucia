...
public class ActionExecution
{
	...
	{
		AsyncTask<Void, Void, ActionResult> requestPermissionTask = new AsyncTask<Void, Void, ActionResult>()
		{
			@Override
	...
	public void executeAction()
	{
		...
		{
			AsyncTask<Void, Integer, Void> executeActionTask = new AsyncTask<Void, Integer, Void>()
			{
				@Override
			...
			executeActionTask.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
		}
	}
	...
	private void continueExecNextAction(CompositeAction composite, int requestCode, int resultCode, Intent result, Activity currentActivity, boolean continueFromPendings)
	{
		...
		else
		{
			...
			if (!composite.isCurrentActionFail() && continueFromPendings) {
				...
				// if currentActionFail, and isNested, remove caller from pendings
				removeLastPendingEventFromPendings();
			}
