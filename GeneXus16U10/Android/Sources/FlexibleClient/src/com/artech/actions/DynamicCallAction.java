...
public class DynamicCallAction extends Action {
	...
	@NonNull
	public static DynamicCallAction redirect(UIContext context, Entity data, String dynamicCall) {
		return new DynamicCallAction(context, data, dynamicCall, false);
	}

	@NonNull
	public static DynamicCallAction redirect(UIContext context, Entity data, String dynamicCall, boolean supportNominatedParameters) {
		return new DynamicCallAction(context, data, dynamicCall, supportNominatedParameters);
	}
	...
	private DynamicCallAction(UIContext context, Entity entity, String dynamicCall, boolean supportNominatedParameters) {
	...
	private void populateAction(String callString, ActionDefinition def)
	{
		...
		else if (objType.equalsIgnoreCase("eo"))
		{
			...
			if (objNameParts.length > 1) {
				...
				{
					if (Services.Application.getPattern(objNameAux) != null) {
						objName = objNameAux;
						break;
					}
					objSeparIndex++;
