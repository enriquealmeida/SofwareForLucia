...
public class GxLocationEdit extends RelativeLayout implements IGxEdit, IGxEditThemeable, IGxHandleActivityResult
{
	...
	private void setMyLocation()
	{
		new WithPermission.Builder<Void>((Activity)getContext())
				...
				.onSuccess(() -> {
					Location location = MyApplication.getInstance().getGeoLocationHelper().getLastKnownLocation();
					if (location != null)
	...
	{
		Intent placePickerIntent = GooglePlaceHelper.buildIntent(mContext, getInternalValue());
	...
	private void callBasicGeolocationPicker()
	{
		...
		ActivityLauncher.callLocationPicker(mContext, mMapType, getInternalValue());
	}
