...
public class GxWebView extends WebView implements IGxEdit, IGxEditThemeable
{
	...
	public class MyWebViewClient extends WebViewClient
	{
		...
		{
			//Services.Log.info("shouldOverrideUrlLoading", " " + url);
			if (url != null)
			{
				...
				if (!event.fire(Arrays.asList(url.toString(), false), mCoordinator, new ExternalObjectEvent.IEntityEventListener() {
					@Override
	...
	public void navigate(String url)
	{
		...
		if (!Strings.starsWithIgnoreCase(url, "http:") && !Strings.starsWithIgnoreCase(url, "https:") && !Strings.starsWithIgnoreCase(url, "file:"))
			url = MyApplication.getApp().linkObjectUrl(url);
		...
		if (!MyApplication.getApp().getAllowWebViewExecuteLocalFiles() && Strings.starsWithIgnoreCase(url, "file:"))
			return;

		// load url
		if (MyApplication.getApp().getShareSessionToWebView())
			new WebViewTaskGxWebView().execute();
		else
			super.loadUrl(url);
	...
	public void loadHtml(String html, ThemeClassDefinition themeClass)
	{
		...
		loadDataWithBaseURL(MyApplication.getApp().getAPIUri(), htmlToLoad, "text/html", "utf-8", "about:blank");
