...
public class GxDateTimeEdit extends LinearLayout implements IGxEdit, IGxEditThemeable, IGxLocalizable, IGxOverrideThemeable
{
	...
	{
		mDateButton = createButton(R.string.GXM_Date);
		mTimeButton = createButton(R.string.GXM_Time);
	...
	private TextView createButton(@StringRes int hintTextRes)
	{
	...
	public void setGxValue(String value)
	{
		//noinspection IfCanBeSwitch
		if (DataTypes.isTime(mDataType, mLength))
			mValue = Services.Strings.getDateTime(value, true, mDecimals==12);
		else if (mDataType.equals(DataTypes.DATE))
			mValue = Services.Strings.getDate(value);
		else
			mValue = Services.Strings.getDateTime(value, false, mDecimals==12);
	...
	private void updateText()
	{
		...
		{
			// Clear texts so that hint text is shown.
			mDateButton.setText("");
			mTimeButton.setText("");
		}
	}
