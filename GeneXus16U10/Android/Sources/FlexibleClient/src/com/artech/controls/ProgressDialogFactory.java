...
public class ProgressDialogFactory
{
	...
	public static void applyThemeToIndicator(ThemeClassDefinition progressThemeClassDefinition, final AlertDialog dialog, boolean showOnlyAnimation )
	{
		if (progressThemeClassDefinition!=null)
		{
			...
			{
				// change background in 4.x devices
				if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.KITKAT && !showOnlyAnimation)
				{
					// Set color  to custom, panel with animation and text
					int dialogSubViewId = dialog.getContext().getResources().getIdentifier("custom", "id", "android");
					View dialogSubView = dialog.findViewById(dialogSubViewId);
					if (dialogSubView != null)
						dialogSubView.setBackgroundColor(backgroundColorId);

					// Set color  to top panel , not work ok, has different size to custom. topPanel big, title_template small.
					//dialogSubViewId = dialog.getContext().getResources().getIdentifier("title_template", "id", "android");
					//dialogSubViewId = dialog.getContext().getResources().getIdentifier("topPanel", "id", "android");
					//dialogSubView = dialog.findViewById(dialogSubViewId);
					//if (dialogSubView != null)
					//	dialogSubView.setBackgroundColor(backgroundColorId);
				}
				else
