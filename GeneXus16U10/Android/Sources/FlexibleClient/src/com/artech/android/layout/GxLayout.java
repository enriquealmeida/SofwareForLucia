...
import com.artech.controls.ThemedOverrideViewHelper;
...
public class GxLayout extends ViewGroup implements IGxLayout
{
	...
	public void updateChildLayoutParams(CellDefinition cell, LayoutItemDefinition item, View view)
	{
		...
		if (lp!=null)
		{
			...
			lp.width = item.getDesiredWidth(); // Margin already added when theme was applied
			...
			if (item.hasAutoGrow())
			{
				...
				if (!(view instanceof GridContainer))
				{
					...
					view.setMinimumHeight(item.getDesiredHeight()); // Margin already added when theme was applied
				}
			...
			{
				lp.height = item.getDesiredHeight(); // Margin already added when theme was applied
			}
		}
		else
		{
