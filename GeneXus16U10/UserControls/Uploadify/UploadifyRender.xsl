...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'Uploadify'">
        <xsl:call-template name="RenderUploadify"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- Uploadify design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RenderUploadify">
  <span atomicselection="true">
      <xsl:call-template name="AddStyleAttribute"/>
      	<table width="109" height="30" border="0">
			<TR>
				<TD>            
					<xsl:attribute name="background">
						<xsl:value-of select='gxca:GetMyPath()'/>
						<xsl:text>\UploadifyRender.JPG</xsl:text>
					</xsl:attribute>
