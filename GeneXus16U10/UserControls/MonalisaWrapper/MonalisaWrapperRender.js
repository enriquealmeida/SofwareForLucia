	...
	this.SetMonalisaTokenCollection = function(data)
	{
		...
		//if(data.length > 0){
			...
			for(var i=0; i<data.length;i++){
				var newToken = new Monalisa.Token(data[i].type, data[i].value);
				newToken.methodClassName = data[i].methodClassName;
				this.MonalisaTokenCollection[this.MonalisaTokenCollection.length] = newToken;
			}	
		//}
		...
		if (this.FirstLoad) {
	        ...
	        if (this.MonalisaFieldCollection) {
	            for (var i = 0; i < this.MonalisaFieldCollection.length; i++) {
	                ...
	                var Field = new Monalisa.Field(this.MonalisaFieldCollection[i].Name, this.MonalisaFieldCollection[i].Description, this.MonalisaFieldCollection[i].Type, FieldDomain);
					...
					Field.class = this.MonalisaFieldCollection[i].Class;
					Field.methodClassName = this.MonalisaFieldCollection[i].methodClassName;
					if(Field.class == "method"){
						this.editor.AddMethod(Field);
					}else{
	        ...
	        if (this.MonalisaListCollection) {
	            for (var i = 0; i < this.MonalisaListCollection.length; i++) {
	                ...
	                var List = new Monalisa.List(this.MonalisaListCollection[i].Name, this.MonalisaListCollection[i].Description, this.MonalisaListCollection[i].Type);
	        ...
	        if (this.MonalisaResultCollection) {
	            for (var i = 0; i < this.MonalisaResultCollection.length; i++) {
	                ...
	                var Result = new Monalisa.Result(this.MonalisaResultCollection[i].Name, this.MonalisaResultCollection[i].Description);
	                var ResultDomain = this.GetDomainObj(Domains, this.MonalisaResultCollection[i].DomainId);
	                if (ResultDomain || this.MonalisaResultCollection[i].InputCode) {
	                    var arg = new Monalisa.Arg("code", "");
						if (ResultDomain){
							arg.domain = ResultDomain;
						}	
	                    Result.AddArg(arg);
	                }
					if (this.MonalisaResultCollection[i].HasPercent) {
