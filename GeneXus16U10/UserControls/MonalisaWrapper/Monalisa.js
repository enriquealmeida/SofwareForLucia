...
include_js(basepath+"jquery-1.9.1.min.js");
include_js(basepath+"jquery.autocomplete.min.js");
include_js(basepath+"dom-to-image.js");
...
Monalisa.Editor.prototype.GetEditor = function(){
	return this;
}
...
Monalisa.Editor.prototype.CreateHTML = function(){
    ...
    if (!this.ReadOnly){
		...
		if (this.Methods.length > 0){
            var groupClases = new Monalisa.ToolboxGroup("Clases");
	        toolbox.AddGroup(groupClases);
    	
			var methodClassName = "";
			for(var i=0; i< this.Methods.length; i++){
			
				if (this.Methods[i].methodClassName != methodClassName){
					methodClassName = this.Methods[i].methodClassName;
					var group = new Monalisa.ToolboxGroup("Clase:"+methodClassName);
					toolbox.AddGroup(group);
				}
			
                var item = new Monalisa.ToolboxItem(this.Methods[i].name, this.Methods[i].description, "method");
                item.methodClassName = this.Methods[i].methodClassName;
	            group.AddItem(item);
	        }
        }
    ...
    //	group.AddItem(item);
		...
		Monalisatag.appendChild(tr2);
		var td21 = document.createElement('td');
		td21.style.align="left";
		td21.style.verticalAlign="top";
		td21.style.width="1px"
		tr2.appendChild(td21);
		
	
	    td1.appendChild(toolbox.CreateHTML());	
		td21.appendChild(menu.CreateHTML());
	...
	//Agrega menu contextual para exportar como imagen
	var menuToImage = new Monalisa.Menu();
	menuToImage.showposition = "mouse";
	this.menuToImage = menuToImage;
	menuToImage.Editor = this;
	
	menuToImage.AddItem(new Monalisa.MenuItem("Guardar Imagen", to_image));
	
	workspace.appendChild(menuToImage.CreateHTML());
	addEvent(vif, 'contextmenu', function(ev) { ev.preventDefault();this.ObjRef.GetEditor().menuToImage.Show();return false;});
	...
	var toimageLink = document.createElement('a');
	toimageLink.id = "toimageLink";
	toimageLink.download = "Regla.png"
	td1.appendChild(toimageLink);
...
to_image = function(){
	var node = this.ObjRef.GetEditor().DOMRef.getElementsByClassName("EditorIf")[0];
	domtoimage.toPng(node)
    .then(function (dataUrl) {
		var toimageLink = document.getElementById("toimageLink");
		toimageLink.href = dataUrl;
		toimageLink.click();
	});
	this.ObjRef.menu.Hide();
}	
...
Monalisa.Editor.prototype.AddMethod = function(vMethod){
    this.Methods[this.Methods.length] = vMethod;
}
...
Monalisa.Menu.prototype.Show = function(){
	if (this.status == "visible"){
		...
		if(this.showposition == "down"){
			var calcularTop = event.currentTarget.getBoundingClientRect().top + event.currentTarget.offsetHeight - event.currentTarget.ObjRef.GetEditor().DOMRef.getBoundingClientRect().top;
			var calcularLeft = event.currentTarget.getBoundingClientRect().left
		}else{
			calcularTop = event.layerY;
			calcularLeft = event.currentTarget.getBoundingClientRect().left + event.layerX - event.currentTarget.ObjRef.GetEditor().DOMRef.getBoundingClientRect().left;
		}
		this.DOMRef.style.top = calcularTop+"px";
...
closeMenu = function(){
	//alert('click');
	if (this.Editor.menu && this.Editor.menu.status == "starting"){
		this.Editor.menu.status = "visible"
	}
	else{
		if (this.Editor.menu){
			this.Editor.menu.Hide();
		}	
		if (this.Editor.menuToImage){
			this.Editor.menuToImage.Hide();
		}
	}	
}
...
Monalisa.MenuItem.prototype.GetEditor = function(){
	return this.menu.Editor;
}
...
Monalisa.Node.prototype.DropNewElement = function(vsource){
	...
	var sourceMethodClassName = vsource.methodClassName;
	...
	if (sourcetype == "method"){
		...
		var newmethod = new Monalisa.Node("methodText", sourcename);
		newmethod.methodClassName = sourceMethodClassName;
		var argument11 = new Monalisa.Node("argument", "argument11");
		argument11.domainType = "Text";
		newmethod.AddChild(argument11);
		argument1.AddChild(newmethod);
...
Monalisa.Node.prototype.CreateHTML = function(){
    ...
    if (this.type == "constraint"){
		...
		addEvent(constarinttext, 'click', function(ev) { ev.preventDefault();MenuChangeConstraintShow();return false;});
...
//		newNode.appendChild(document.createElement("br"));
			...
			if(vsuggestId){
				...
				if (value.value != ""){
					$((function(x, y) { return function() {
						...
						$.getJSON("com.riesgodian.riesgo.selectividad.awsdominiovalores?"+y+" ,"+x.value, function(result){
				...
				$((function(x, y) { return function() {
					...
					$(x).autocomplete({
						...
						serviceUrl: 'com.riesgodian.riesgo.selectividad.awsdominiovalores?'+y+',',
						minChars: 3,
						width:600,
    ...
    if (this.type == "methodText"){
		//span para nombre de method
		var methodClassName = document.createElement("span");        
		methodClassName.id = "className"+this.name;
		methodClassName.className ="methodClassName";
		var text = document.createTextNode(this.methodClassName);
		methodClassName.appendChild(text);
		newNode.appendChild(methodClassName);
		...
		methodName.id = "methodName"+this.name;
		methodName.className ="methodName";
		var text = document.createTextNode(" "+this.name+" ");
		methodName.appendChild(text);
		newNode.appendChild(methodName);
    }
...
MenuChangeConstraintShow = function() {
	if (event.currentTarget.parentNode.ObjRef.GetEditor().menu){
...
Monalisa.Node.prototype.Compile = function(){
    ...
    switch(this.type){
		...
		case "functionText":
			...
			break;	
		case "methodText":
			var newtoken = new Monalisa.Token('methodText', this.name);
			newtoken.methodClassName = this.methodClassName;
			tokenlist[tokenlist.length] = newtoken;
			var newtoken = new Monalisa.Token('parizq', '(');
			tokenlist[tokenlist.length] = newtoken;
			var tokenlistaux = this.Children[0].Compile();
...
//Producciones con 2 elementos
        if (vToken2){
			...
			//result :: resultado code
		    if (vfase > 1 && (vToken1.type == 'resultado' && vToken2.type == 'code')){
		        ...
		        var newnode = new Monalisa.Node("resultado", vToken1.value, vresultadodescription);
			    var newnodearg = new Monalisa.Node('code', vToken2.value);
                for (var j = 0; j < this.Results.length; j++) {
		            if (this.Results[j].name == vToken1.value) {
...
//Producciones con 3 tokens
		if (vToken3){
            ...
            //argument :: ( argument )
            if (vfase > 1 && (vToken1.type == 'parizq' && vToken2.type == 'argument' && vToken3.type == 'parder')){
            ...
            //result :: resultado code porcent
		    if (vToken1.type == 'resultado' && vToken2.type == 'code' && vToken3.type == 'percent'){
		...
		if(vToken4){
			...
			//argument :: methodText parizq argument1 parder 
            if ( vToken1.type == 'methodText' && vToken2.type == 'parizq' && vToken3.type == 'argument' && vToken4.type == 'parder'){
		        
		        var newargument = new Monalisa.Node("argument", "");
		        var newmethod = new Monalisa.Node("methodText", vToken1.value);
				newmethod.methodClassName = vToken1.methodClassName;
		        newmethod.AddChild(vToken3.tree);
		        newargument.AddChild(newmethod);
