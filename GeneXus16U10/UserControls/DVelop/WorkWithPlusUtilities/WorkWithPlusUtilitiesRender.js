...
function DVelop_WorkWithPlusUtilities() {
			...
			return function (e) {
				...
				if (thisC.EnableConvertComboToBootstrapSelect) {
					if (wwp_currentDDO != null) {
						thisC.ApplyBootstrapSelect_resetSelection(wwp_currentDDO.ddoControl, wwp_currentDDO.target);
						wwp_currentDDO = null;
					}
			...
			if (!$(h).hasClass("selectpicker_DV")) {
				...
				c.on("changed.bs.select", function (j, i, l, k) {
					if ($(this).find('>select').hasClass('selectpicker_DV')) {
						...
						wwp_currentDDO = { 'ddoControl': this, 'target': j.target };
					}
				});
	...
	this.ApplyBootstrapSelect_resetSelection = function (ddoControl, target) {
		$(target).val(0);
		$(target).selectpicker_DV("refresh");
		this.ApplyBootstrapSelect_updateDDO(ddoControl);
