...
sub lib_osds_usm_supported
{
  ...
  if (($id eq 2) && ($major == 6) && ($minor == 2) && ($prod_type == 3))
  {
    ...
    {
      my ($OSVERSION) = `systeminfo | findstr /B /C:"OS Version"`;
      chomp($OSVERSION);
      $version = acfslib::trim(substr($OSVERSION, index($OSVERSION, ':') + 1));
    }
    ...
    # For Windows Server 2012 you got 6.2.9200
    elsif (defined($version) &&
        $version =~ /6.2/)
    {
  ...
  elsif (($id eq 2) && ($major == 6) && ($minor == 2) && ($prod_type == 1))
  {
    ...
    # For Windows 8 you got 6.2.9200
    elsif (defined($version) &&
           $version =~ /6.2/)
    {
      $OS_TYPE = "Windows 8";
    }
    # For Windows 10 got 10.0.10240
    elsif (defined($version) &&
           $version =~ /10.0/)
    {
      $OS_TYPE = "Windows 10";
