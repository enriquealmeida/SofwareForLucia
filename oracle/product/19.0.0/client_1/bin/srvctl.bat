...
@set JREDIR=c:\app\oracle\product\19.0.0\client_1\jdk
@set JRE="%JREDIR%\bin\java"
@set JREJAR=%JREDIR%\jre\lib\rt.jar
@set JLIB=c:\app\oracle\product\19.0.0\client_1\jlib
set EONSJAR=c:\app\oracle\product\19.0.0\client_1\ons\lib\ons.jar
@set GNSJAR=%JLIB%\gns.jar
set ANTLRJAR=c:\app\oracle\product\19.0.0\client_1\srvm\jlib\antlr-3.3-complete.jar
@set PATH=c:\app\oracle\product\19.0.0\client_1\bin;%PATH%
set CLSCEJAR=%JLIB%\clsce.jar
set CHACONFIGJAR=%JLIB%\chaconfig.jar
set JDBCJAR=c:\app\oracle\product\19.0.0\client_1\jdbc\lib\ojdbc6.jar
...
if (%ORATST_SRVCTL11%)==() (
   set CLASSPATH="c:\app\oracle\product\19.0.0\client_1\srvm\jlib\srvctl.jar;%JLIB%\srvm.jar;%JLIB%\srvmhas.jar;%JLIB%\srvmasm.jar;%JLIB%\supercluster-common.jar;%JLIB%\supercluster.jar;%LDAPJAR%;%JLIB%\netcfg.jar;%JREJAR%;%EONSJAR%;%GNSJAR%;%ANTLRJAR%";%CLSCEJAR%;%CHACONFIGJAR%;%JDBCJAR%;%JWCCREDJAR%
) else (
...
set ORACLE_HOME_PROP=c:\app\oracle\product\19.0.0\client_1

Rem - If CRSHOME, unset ORACLE_HOME 
if exist c:\app\oracle\product\19.0.0\client_1\bin\crsctl.exe (
  set ORACLE_HOME=
)
