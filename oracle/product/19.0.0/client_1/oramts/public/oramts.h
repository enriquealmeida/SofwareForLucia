/*
 * $Header: ntoramts/oramts/resdisp/public/oramts.h /main/4 2012/06/25 23:37:35 yohu Exp $ oramts.h
 */

/* Copyright (c) 1986, 2012, Oracle and/or its affiliates. 
All rights reserved. */
...
NOTES
  None
MODIFIED 
    yohu       02/22/12 - support proxy authentication
    yohu       08/08/08 - PSPE support
    vraja      05/09/03 - 
...
/*---------------------------------------------------------------------------*/
/* Name      : OraMTSSvcGetEx2()                                             */
/* Purpose   : Obtain a pooled connection from the OCI connection pool.      */
/* Arguments : text         *lpUname: (IN ) user name for session            */
/*             text         *lpPsswd: (IN ) password for session             */
/*             text         *lpPclnt: (IN ) proxy client for session         */
/*             text         *lpDbnam: (IN ) database alias for connection    */
/*             OCISvcCtx   **pOCISvc: (OUT) ptr to OCI svc context handle.   */
/*             OCIEnv      **pOCIEnv: (OUT) ptr to OCI enironment handle.    */
/*             ub4           ConFlgs: (IN ) connection flags                 */
/*             ISOLATIONLEVEL isoLvl: (IN ) transaction isolation level. See */
/*               transact.h for details. Lvels SERIALIZABLE, REPEATABLEREAD, */
/*               and READCOMMITTED are supported. The first two map to       */
/*               Oracle's serializable transactions. The last one corresponds*/
/*               to the default Oracle isolation level of read-committed.    */
/* Returns   : KPNTERR_NOERROR on success.                                   */
/* Notes     : This call returns a pooled OCI connection to the caller. If   */
/*             the caller is a transactional MTS component, currently in a   */
/*             MSDTC controlled transaction, we will attempt to enlist the   */
/*             session in the transaction unless the caller explicitly asks  */
...
/*             will not attempt to enlist.                                   */
/*---------------------------------------------------------------------------*/
DWORD OraMTSSvcGetEx2(text *lpUname, text *lpPsswd,
                      text *lpPclnt, text *lpDbnam,
                      OCISvcCtx **pOCISvc, OCIEnv **pOCIEnv, 
                      ub4 ConFlgs, ISOLATIONLEVEL isoLvl);
...
/*---------------------------------------------------------------------------*/
/* Name      : OraMTSEnlCtxGet2()                                            */
/* Purpose   : Create an enlistment context for a non-pooled OCI connection; */
/*               The enlistment context then may be used to promote a local  */
/*               txn started on the connection to a distributed one.         */
/* Arguments : text*      pUname :(IN ) database username for connection.    */
/*             text*      pPsswd :(IN ) database password for connection.    */
/*             text*      pDbnam :(IN ) database connect str for connection. */
/*             OCISvcCtx* pOCISvc:(IN ) OCI service context for pooled conn. */
/*             OCITrans*  pOCITxn:(IN ) OCI txn handle                       */
/*             OCIError*  pOCIErr:(IN ) OCI error handle for errors.         */
/*             ub4        dwFlags:(IN ) enlistment flags. 0 is the only      */
/*               value currently allowed.                                    */
/*             void**     pCtxt  :(OUT) enlistment context to be created.    */
...
/*             ub4        dwFlags: (IN ) enlistment flags                    */
/* Returns   : KPNTERR_NOERROR on success.                                   */
/* Notes     : This call sets up an enlistment context for a non-pooled OCI  */
/*             connection. This call can be issued after the caller has      */
/*             established the OCI connection to the database, and after     */
/*             a local transaction may have started on this connection.      */
/*             If a local transaction has begun, the caller should pass in   */
/*             the OCI txn handle representing the local transaction.        */
/*             If no local txn was started, the caller should pass a NULL    */
/*             OCI txn handler. Once the enlistement context is created,     */
/*             the context can then be passsed to be OraMTSPromoteTxn()      */
/*             calls. Prior to tearing down the OCI connection,              */
/*             OraMTSEnlCtxRel() must be called to destroy the enlistment    */
/*             context.                                                      */
/*---------------------------------------------------------------------------*/
DWORD OraMTSEnlCtxGet2(text* lpUname, text* lpPsswd, text* lpDbnam,  
                       OCISvcCtx* pOCISvc,
                       OCITrans*  pOCITrans,
                       OCIError*  pOCIErr,
                       ub4 dwFlags, void** pCtxt);
...
/*---------------------------------------------------------------------------*/
/* Name      : OraMTSEnlCtxGet3()                                           */
/* Purpose   : Create an enlistment context for a non-pooled OCI connection. */
/* Arguments : text*      pUname :(IN ) database username for connection.    */
/*             text*      pPsswd :(IN ) database password for connection.    */
/*             text*      pPclnt :(IN ) database proxy client for connection.*/
/*             text*      pDbnam :(IN ) database connect str for connection. */
/*             OCISvcCtx* pOCISvc:(IN ) OCI service context for pooled conn. */
/*             OCITrans*  pOCITxn:(IN ) OCI txn handle                       */
/*             OCIError*  pOCIErr:(IN ) OCI error handle for errors.         */
...
/* Notes     : This call sets up an enlistment context for a non-pooled OCI  */
/*             connection. This call can be issued after the caller has      */
/*             established the OCI connection to the database, and has begun */
/*             a local transaction on this connection. If a local transaction*/
/*             has begun, the caller should pass in the OCI txn handle for   */
/*             the local transaction. If there is no such a local txn, the   */
/*             caller should pass a NULL OCI txn handler instead.            */
/*             Once the enlistement context is created, the context can then */
/*             be passsed to be OraMTSPromoteTxn() calls. Prior to tearing   */
/*             down the OCI connection kpntenlistctxrel() must be called to  */
/*             destroy the enlistment context.                               */
/*---------------------------------------------------------------------------*/
DWORD OraMTSEnlCtxGet3(text* lpUname, text* lpPsswd, 
                       text* lpPclnt, text* lpDbnam,  
                       OCISvcCtx* pOCISvc,
                       OCITrans*  pOCITrans,
                       OCIError*  pOCIErr,			
...
/*---------------------------------------------------------------------------*/
/* Name      : OraMTSPromoteTxn()                                            */
/* Purpose   : Promote a local transaction to distributed one, and enlist an */
/*             non-pooled OCI connection in the promoted MSDTC-transaction.  */
/* Arguments : void*           pCtxt: (IN ) enlistment ctxt for the OCI conn */
/*             ISOLATIONLEVEL isoLvl: (IN ) transaction isolation level. See */
/*               transact.h for details. Lvels SERIALIZABLE, REPEATABLEREAD, */
/*               and READCOMMITTED are supported. The first two map to       */
/*               Oracle's serializable transactions. The last one            */
/*               corresponds to the default Oracle isolation level of        */
/*               read-committed.                                             */
/*             ULONG    timeout_sec   (IN) timeout in seconds; if 0, then    */
/*               use system default setting for the promoted MSDTC txn       */
/*             void**   pTrans: (OUT ) reference to MSDTC txn object         */
/*             byte**   token:  (OUT ) propagation token                     */
/*             ULONG*   token_size:   (OUT ) propagation token size          */
/* Returns   : ORAMTSERR_NOERROR on success.                                 */
/* Notes     : This call is used by clients with non-pooled OCI connections  */
/*             to enlist these connections which may have active local       */
/*             transactions in MSDTC-coordinated transations.                */
/*             The client passes in the opaque reference to the enlistment   */
/*             ctxt representing the OCI connection where a local txn could  */
/*             have been existing. The local transaction is then promoted    */
/*             to a distributed MSDTC transaction, and the connection is     */
/*             then enlisted in the promoted MSDTC transaction.              */
/*---------------------------------------------------------------------------*/
DWORD OraMTSPromoteTxn(void*           pCtxt,  
                       ISOLATIONLEVEL  isoLvl,
                       ULONG           timeout_sec,
                       void**          pTrans,
                       byte**          token_return, 
                       ULONG*          token_size);
...
/*---------------------------------------------------------------------------*/
/* Name      : OraMTSCommitPromotedTxn()                                     */
/* Purpose   : Commit a promoted MSDTC transaction.                          */
/* Arguments : void* pTrans (IN)           reference to a promoted MSDTC txn */
/* Returns   : ORAMTSERR_NOERROR on success.                                 */
/*---------------------------------------------------------------------------*/
DWORD OraMTSCommitPromotedTxn(void* pTrans);

/*---------------------------------------------------------------------------*/
/* Name      : OraMTSAbortPromotedTxn()                                      */
/* Purpose   : Abort a promoted MSDTC transaction.                           */
/* Arguments : void* pTrans (IN)           reference to a promoted MSDTC txn */
/* Returns   : ORAMTSERR_NOERROR on success.                                 */
/*---------------------------------------------------------------------------*/
DWORD OraMTSAbortPromotedTxn(void* pTrans);
